...
class Planner:
    ...
    def mark_step_complete(self, state: AgentState, step_id: int) -> None:
        for step in state.current_plan:
            if step["step_id"] == step_id:
                step["status"] = "completed"
                break

    def mark_step_failed(self, state: AgentState, step_id: int, reason: str) -> None:
        for step in state.current_plan:
            if step["step_id"] == step_id:
                step["status"] = "failed"
    ...
    def replan_on_failure(self, state: AgentState, failed_step_id: int) -> None:
        """
