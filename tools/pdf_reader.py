...
def run_sarvam_ocr_chunked(
    file_path: str, api_key: str, page_limit: int = 100, language: str = "hi-IN"
) -> List[str]:
    import fitz
    import tempfile
    import os
...
def read_pdf_directory(
    directory_path: str, page_limit: int = 100
...
def read_pdf_file(file_path: str, page_limit: int = 100) -> List[PatientDocument]:
    ...
    if not has_text and SARVAMAI_API_KEY:
        print(
            f"Scanned PDF detected. Using Sarvam AI Document Intelligence for OCR: {file_path}"
        )
        try:
            ...
            print(
                f"Sarvam AI OCR failed: {exc}. Falling back to standard PyMuPDF/Gemini reader."
            )
    ...
    for page_num in range(pages_to_read):
        try:
            ...
            if (
                len(text.strip()) < 50
                and getattr(SETTINGS, "ocr_fallback", True)
                and GOOGLE_API_KEY
            ):
                try:
